...
DECLARE
  ...
  CURSOR cur_closed_orders IS
    SELECT CAST(o.order_id AS VARCHAR(10)) order_id, 
          CAST(pi.product_name AS VARCHAR(100)) product_name, oi.product_id, oi.quantity, TO_DATE(CAST(o.order_date AS DATE), 'DD-MON-YYYY') AS order_date
    FROM 
...
BEGIN
  ...
  SELECT CAST(o.order_id AS VARCHAR(10)) order_id, 
        pi.product_name , oi.product_id, oi.quantity, TO_DATE(CAST(o.order_date AS DATE), 'DD-MON-YYYY') AS order_date
